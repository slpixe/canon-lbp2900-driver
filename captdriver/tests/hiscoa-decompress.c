...
/* Decoder is a test oracle, not part of the installed filter. */
static void trace(FILE *stream, const char *format, ...) {
...
unsigned n;
	...
	while (! end && state.bitpos < 8 * state.input_size) {
		...
		switch (group) {
		case 0:
			trace(stderr, "  LONGREP0 (LINESIZE=%u, L0=%u) ", line_size, state.origin_0);
			copy_block(&state, state.origin_0);
			break;
		case 1:
			n = read_number(&state, 4);
		        {
				trace(stderr, "  REPBYTE (%u) stash={", n);
				for (i=0; i<16; i++) trace(stderr, " %.2x", state.bytes[i]);
				trace(stderr, " }\n");
			}
		...
		case 2:
			if (read_bit(&state)) {
				...
				trace(stderr, "  LONGREP2 (LINESIZE=%u, L2=%u)", line_size, state.origin_2);
				copy_block(&state, state.origin_2);
				swap(&state.origin_2, &state.origin_0);
			}
			break;
		case 3:
			trace(stderr, "  LONGREP3 (L3=%u) ", state.origin_3);
			copy_block(&state, state.origin_3);
			break;
		case 4:
			trace(stderr, "  LONGREP4 (L4=%u) ", state.origin_4);
			copy_block(&state, state.origin_4);
			break;
		case 5:
			trace(stderr, "  LONGREP5 (L5=%u) ", state.origin_5);
